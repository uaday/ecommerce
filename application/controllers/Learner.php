...
class Learner extends CI_Controller {
    ...
    public function index($offset = 0)
    {
        ...
        $limit = 12;
        $result = $this->front_learner_model->show_all_learner($limit, $offset);
        $data['learner_list'] = $result['rows'];
        $data['num_results'] = $result['num_rows'];
        // load pagination library
        $this->load->library('pagination');
        $config = array();
        $config['base_url'] = site_url("learner/index");
    ...
    public function details_learner($learner_id=0)
    {
        $data['learner']=$this->front_learner_model->find_learner_by_learner_id($learner_id);
        $data['hero_header'] = TRUE;
        $data['main_content'] =$this->load->view('front_end_view/learner/view_details_learner', $data, TRUE);
        $this->load->view('front_end_view/master',$data);
    }
}
