...
class About_slider extends CI_Controller {
...
//        $this->load->view('super_admin_view/login_register/view_login');
    ...
    {
        $this->load->view('super_admin_view/about_slider/view_add_slider');
    }
    public function add_new_slider()
    {
        ...
        if (!empty($_FILES['slider_image']['name'])) {
            $uploadPath = 'upload/about_slider_image/';
            ...
            if ($this->upload->do_upload('slider_image')) {
                ...
                $result=$this->about_slider_model->add_new_slider($data);
                if($result==1)
                {
                    $this->session->set_userdata('add_new_slider',"Slider Successfully Added");
                    redirect(base_url().'portal/about_slider/manage_slider');
                }
            }
        }
    }
    public function edit_slider_details()
    {
        ...
        $result=$this->about_slider_model->update_slider($data);
        if($result==1)
        {
    ...
    {
        $data['sliders']=$this->about_slider_model->show_all_slider();
        $this->load->view('super_admin_view/about_slider/view_manage_slider',$data);
    }
    public function edit_slider($slider_id=0)
    {
        $data['sliders']=$this->about_slider_model->select_slider_by_slider_id($slider_id);
        $this->load->view('super_admin_view/about_slider/view_edit_slider',$data);
    }
    public function delete_slider($slider_id=0)
    {
        $result=$this->about_slider_model->delete_slider($slider_id);
        if($result=='1')
        {
            redirect(base_url().'portal/about_slider/manage_slider');
        }
    ...
    {
        $this->about_slider_model->active_slider($slider_id);
        redirect(base_url().'portal/about_slider/manage_slider');
    }
    public function block_slider($slider_id=0)
    {
        $this->about_slider_model->block_slider($slider_id);
        redirect(base_url().'portal/about_slider/manage_slider');
    }
}
